Schema.intersect([
    Schema.object({
        ...
        lora_type: Schema.union(["lora", "lokr", "glokr", "tlora", "lora_fa", "vera", "loha", "bokr", "bora", "gsokr", "glora_boft"]).default("lora").description("适配器类型。常规训练建议选择 LoRA；glokr/bokr/bora/gsokr/glora_boft 为本地扩展 LyCORIS 算法（仅 Linear 层，Conv 层自动跳过）"),
    ...
    Schema.intersect([
        ...
        Schema.union([
            ...
            Schema.object({
                ...
                lycoris_ext_hint: Schema.string().role('textarea').default("GLoKR = LoKr + GLoRA 融合算法，仅作用于 Linear 层（Conv 层自动跳过，适合 Anima 等 DiT 模型）。下方参数的默认值即画风训练推荐配置（kron_rank=2 + BoRA + 门控 + NKP 初始化），无需再手动填写 network_args，开箱即用；输入侧 G 路径（W₀·A）恒定启用。各算法区别与参数详解见「帮助 → 训练算法说明」").disabled().description("GLoKR 使用说明"),
                ...
                dora_wd: Schema.boolean().default(false).description("启用 DoRA 权重解耦（单向幅度解耦）。与上方 use_bora 二选一，use_bora 开启时无需再开"),
                bypass_mode: Schema.boolean().default(false).description("bypass 前向模式：不重建大矩阵，适合量化底模 / 小 batch；与 DoRA/BoRA 互斥"),
                ...
                module_dropout: Schema.number().step(0.01).min(0).max(1).description("module dropout 概率"),
                rank_dropout_scale: Schema.boolean().default(false).description("对 rank dropout 进行缩放补偿"),
                dropout: Schema.number().step(0.01).min(0).max(1).description("Dropout 概率（GLoKR 仅 bypass 模式生效）"),
                pissa_init: Schema.boolean().hidden(),
                network_dropout: Schema.number().hidden(),
            }),
            Schema.object({
                ...
                lycoris_ext_hint: Schema.string().role('textarea').default("BoKR = LoKr + BoRA 双向权重解耦，仅作用于 Linear 层。下方 dora_wd 开关即 BoRA 双向解耦（默认开启，关闭则退化为普通 LoKr），无需再手写 network_args。各算法区别与参数详解见「帮助 → 训练算法说明」").disabled().description("BoKR 使用说明"),
            ...
            Schema.object({
                ...
                lycoris_ext_hint: Schema.string().role('textarea').default("BoRA = 双向范数解耦的 LoRA/LoCon 变体，仅作用于 Linear 层。下方 dora_wd 开关即 BoRA 双向幅值解耦（默认开启，关闭则退化为普通 LoRA），无需再手写 network_args。各算法区别与参数详解见「帮助 → 训练算法说明」").disabled().description("BoRA 使用说明"),
            ...
            Schema.object({
                ...
                lycoris_ext_hint: Schema.string().role('textarea').default("GloKrSora = GLoKR 的 SoRA 稀疏化变体（实验性），仅作用于 Linear 层。下方 SoRA 参数已可视化（稀疏门控默认开启），无需再手写 network_args。各算法区别与参数详解见「帮助 → 训练算法说明」").disabled().description("GloKrSora 使用说明"),
            ...
            Schema.object({
                ...
                lycoris_ext_hint: Schema.string().role('textarea').default("GLoRA-BOFT = GLoRA 乘性重参数化与 BOFT（蝶形正交变换）的组合算法，仅作用于 Linear 层。下方 BOFT 参数已可视化，无需再手写 network_args。各算法区别与参数详解见「帮助 → 训练算法说明」").disabled().description("GLoRA-BOFT 使用说明"),
