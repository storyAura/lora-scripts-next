Schema.intersect([
    Schema.object({
        ...
        lora_type: Schema.union(["lora", "lokr", "glokr", "tglokr", "tlora", "lora_fa", "vera", "loha", "bokr", "bora", "gsokr", "glora_boft"]).default("lora").description("适配器类型。常规训练建议选择 LoRA；glokr/tglokr/bokr/bora/gsokr/glora_boft 为本地扩展 LyCORIS 算法（仅 Linear 层，Conv 层自动跳过）。tglokr = GLoKR + 时间步门控（T-GLoKR，实验特性）"),
    ...
    Schema.intersect([
        ...
        Schema.union([
            ...
            Schema.object({
                ...
                pissa_init: Schema.boolean().hidden(),
                network_dropout: Schema.number().hidden(),
                train_time_gates: Schema.boolean().hidden(),
                time_gate_dim: Schema.number().hidden(),
            }),
            Schema.object({
                ...
                use_bora: Schema.boolean().default(true).description("BoRA 双向（行/列）权重解耦，画风训练推荐开启。开启后无需再开下方 dora_wd"),
                bora_iters: Schema.number().min(1).step(1).default(2).description("BoRA 行/列交替平衡迭代次数，推荐 2~3；1 为单次平衡（原始行为）"),
                train_gates: Schema.boolean().default(true).description("静态路径门控 + Kronecker 项混合系数（管「这层这条路总体多重要」，与时间门控相乘互补）"),
                init_mode: Schema.union(["nkp", "kaiming"]).default("nkp").description("初始化方式。nkp = Van Loan 主成分对齐初始化（推荐，短周期训练收敛更快）；kaiming = 经典随机初始化"),
                use_g_out: Schema.boolean().default(false).description("输出侧 C·W₀ 路径（约每层 6k 参数），色调/氛围向增强。时间门控下该路径最可能学出「中低噪声段发力」的曲线，值得开一轮对照"),
                g_norm_mode: Schema.union(["frobenius", "spectral"]).default("frobenius").description("G 路径尺度对齐所用范数。spectral 可让各层有效学习率更均匀"),
                lokr_factor: Schema.number().min(-1).default(-1).description("Kronecker 分解因子，与 LoKr 相同。-1 为最均衡分解"),
                use_scalar: Schema.boolean().default(false).description("使用可学习全局缩放（0.01 起步）替代零初始化"),
                decompose_both: Schema.boolean().default(false).description("对 w1 也做低秩分解（默认仅分解 w2）"),
