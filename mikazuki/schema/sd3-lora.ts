Schema.intersect([
    Schema.object({
        ...
        lora_type: Schema.union(["lora", "rslora", "lora_plus", "dora", "delora", "waveft", "deft", "moslora", "lokr", "glokr", "tlora", "lora_fa", "vera", "loha", "bokr", "bora", "gsokr", "glora_boft"]).default("lora").description("适配器类型。常规训练建议 LoRA；各算法的区别、适用场景与专属参数详见「帮助 → 训练算法说明」页"),
    ...
    Schema.object({
        ...
        anima_compile_blocks: Schema.boolean().default(false).description("仅在 Linux + CUDA 上区域编译重复的 Anima block，保留 LoRA 注入和 state_dict 结构；不能与全局 torch_compile、量化、offload 或 block swap 同时使用"),
    ...
    Schema.intersect([
        Schema.object({
            ...
            network_dim: Schema.number().min(1).default(16).description("网络维度，常用 4~128，不是越大越好。T-LoRA 建议 32（动态 rank 需要更大基础维度）；WaveFT 使用专属 waveft_n_frequency，此值只用于目标模块选择兼容"),
        ...
        Schema.union([
            ...
            Schema.object({
                ...
                dropout: Schema.number().step(0.01).min(0).max(1).default(0).description("rsLoRA dropout 概率"),
            ...
            Schema.object({
                ...
                dropout: Schema.number().step(0.01).min(0).max(1).default(0).description("DoRA dropout 概率"),
                rank_dropout: Schema.number().step(0.01).min(0).max(1).description("rank dropout 概率"),
                module_dropout: Schema.number().step(0.01).min(0).max(1).description("module dropout 概率"),
                pissa_init: Schema.boolean().hidden(),
                network_dropout: Schema.number().hidden(),
