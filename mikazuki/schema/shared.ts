    ...
    let data = {
        ...
        LYCORIS_MAIN: Schema.union([
            Schema.object({
                ...
                lycoris_algo: Schema.union(["locon", "loha", "lokr", "ia3", "dylora", "glora", "diag-oft", "boft", "bokr", "bora", "gsokr", "glora_boft"]).default("locon").description('LyCORIS 网络算法。bokr/bora/gsokr/glora_boft 为本地扩展算法：仅作用于 Linear 层（适合 DiT 类模型，Conv 层自动跳过）'),
        ...
        ANIMA_FAST_LR_OPTIMIZER: Schema.intersect([
            Schema.object({
                learning_rate: Schema.string().default("1e-4").description("总学习率, 在分开设置 U-Net 与文本编码器学习率后这个值失效。"),
                unet_lr: Schema.string().default("1e-4").description("U-Net 学习率"),
                text_encoder_lr: Schema.string().default("1e-5").description("文本编码器学习率"),
        ...
        PREVIEW_IMAGE: Schema.intersect([
            ...
            Schema.union([
                Schema.object({
                    ...
                    positive_prompts: Schema.string().role('textarea').default('masterpiece, best quality, 1girl, solo').description("Prompt。每行一条 = 一张预览图（第1行 A、第2行 B…）"),
