...
ALGO_REGISTRY: Dict[str, AlgoSpec] = {
    ...
    # --------------------------------------------------------------------------
    # GLoKR 算法注册项 (匹配 GLoKRModule)
    # --------------------------------------------------------------------------
    "glokr": AlgoSpec(
        name="glokr",
        description="Generalized Kronecker-product adapter with optional G-path and BoRA/DoRA weight decomposition.",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
        ...
        notes="Optimized for DiT and Transformer architectures. Set use_g=False to fall back to LoKr.",
    ),
    # 保留 glora_lokr 作为 glokr 的兼容别名
    "glora_lokr": AlgoSpec(
        name="glora_lokr",
        description="Alias for glokr algorithm.",
        supported_args=(
            "dim",
            "alpha",
            "dropout",
        ...
        notes="Designed for linear-only architectures such as Diffusion Transformers (DiT).",
    ),
    "full": AlgoSpec(
        name="full",
