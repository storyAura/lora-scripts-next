        ...
        def create_single_module(
            ...
            **kwargs,
            ...
            try:
                ...
                # Linear-only algos (glokr/bokr/gsokr/...) raise on conv/norm
                # layers; skip the layer instead of aborting the whole build.
                if "is not supported in" not in str(e):
                    raise
                logger.warning(f"Skip {lora_name} ({module.__class__.__name__}): {e}")
                lora = None
            return lora
